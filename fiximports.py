...
def parse_cmdline():
	...
	parser.add_argument('-i', '--imbalance-ac', default="Imbalance-USD", help="Imbalance account name. Default=Imbalance-USD")
	parser.add_argument('-v', '--version', action='store_true', help="Display version and exit.")
	parser.add_argument('-m', '--use_memo', action='store_true', help="Use memo field instead of description field to match rules.")
	parser.add_argument('-s', '--silent', action='store_true', help="Suppress normal output (except errors).")
	parser.add_argument('-n', '--nochange', action='store_true', help="Do not modify gnucash file. No effect if using SQL.")
	parser.add_argument("ac2fix", help="Full path of account to fix, e.g. Liabilities:CreditCard")
